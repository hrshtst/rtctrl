...
for lib in $LIBS; do
  ...
  # rebuilt with CC=g++) miscompile the roki-fd/zm ODE path as of
  # zm 1.14.5 — rtctrl links the C libs and supplies the C++-only
  # static-member definitions itself (src/milib_cpp_compat.cpp).
  make -C "$dir"
  make -C "$dir" install
done
