...
| Manual motion teaching (`x7_teach`) | passive acquisition or gravity compensation | ✓ wire-level emulator | **✓ WORKS**: torque-off and gravity-compensation teaching both completed on the physical arm 2026-08-28, followed by the simulation-first position/CBP playback workflow; each new taught path still requires inspection and simulation before hardware replay ([history](../records/history.md#manual-teaching-and-playback-validation-2026-08-28), [usage](usage.md#manual-motion-teaching)) |
...
| Servo-side trajectory following (`x7_follow`, sim twin `x7_follow_sim`) | servo-internal loops | ✓ full phase simulation | **✓ WORKS** — operated on the physical arm 2026-08-27 in both modes (position-versus-current-based-position comparison, then the CBP gain sweep on the tabletop pick-approach reference); the selected provisional CBP tuning repeated three times at aggregate RMS ≈ 0.018 rad with no overruns, skipped cycles, or stale commands; those gains are experiment-specific, not a motor default ([history](../records/history.md#current-based-position-gain-decision-2026-08-27), [usage](usage.md#servo-side-trajectory-following)) |
| Textbook computed-torque replay (`x7_track`, sim twin `x7_track_sim`) | [notes](../theory/computed-torque.md) | ✓ configuration-driven planner/taught ZVS replay | **⚠ EXPERIMENTAL, NOT YET VALIDATED**: the renewed app intentionally omits the practical compensators; the former hardened app and its parked campaign remain recorded under `PracticalComputedTorque` ([history](../records/history.md#hardware-campaign-and-the-final-06-scale-cap)) |
